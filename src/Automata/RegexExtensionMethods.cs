...
namespace Microsoft.Automata
{
    ...
    public static class RegexExtensionMethods
    {
        ...
        /// <returns></returns>
        public static RegexMatcher Compile(this Regex regex, string filter = "", params Regex[] regexes)
        {
            return Compile(regex, true, true, filter, false, regexes);
        }
        ...
        /// <returns></returns>
        public static RegexMatcher Compile(this Regex regex, bool keepAnchors, bool unwindLowerBounds, string filter = "", bool isMatchOnly = false, params Regex[] regexes)
        {
           ...
           if (regexes.Length == 0)
            ...
            var others = new SymbolicRegexNode<BDD>[regexes.Length];
            for(var i = 0; i < regexes.Length; i++)
            {
                others[i] = context.RegexConverter.ConvertToSymbolicRegex(regexes[i], keepAnchors, unwindLowerBounds);
            }
            ...
            var partition = conj.ComputeMinterms(filter);
            //var str = regex.ToString();
            //Console.Write(str.Remove(str.Length - 1)+ ";" + partition.Length + "\n");
