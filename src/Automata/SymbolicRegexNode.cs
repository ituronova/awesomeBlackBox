...
namespace Microsoft.Automata
{
    ...
    public class SymbolicRegexNode<S> : ICounter
    {
        ...
        /// </summary>
        /// <returns></returns>
        Automaton<Tuple<Maybe<S>, Sequence<CounterOperation>>> Explore(out ICounter[] countingStates, out SymbolicRegexNode<S>[] stateMap, out List<ICounter> cntrs, bool monadic, int pushLevel = 0)
        {
            var node = (monadic ? this : this.builder.NormalizeGeneralLoops(this));
            var stateLookup = new Dictionary<SymbolicRegexNode<S>, int>();
            var regexLookup = new Dictionary<int, SymbolicRegexNode<S>>();
            cntrs = new List<ICounter>();
            int countCounters = 0;
            int level = 0;
            stateLookup[node] = 0;
            int stateid = 1;
            ...
            while (frontier.IsNonempty)
            {
                ...
                foreach (S a in builder.solver.GetPartition())
                {
                    ...
                    for (int i = 0; i < a_derivs.Count; i++)
                    {
                        int p;
                        var cd = a_derivs[i];
                        if (!stateLookup.TryGetValue(cd.PartialDerivative, out p))
            ...
            stateMap = new List<SymbolicRegexNode<S>>(regexLookup.Values).ToArray();
            ...
            if (monadic)
            {
                ...
                ICounter[] newCounters1 = new ICounter[regexLookup.Count];
                ...
                Func<int, int, Sequence<CounterOperation>, Sequence<CounterOperation>> F = (s, t, ops) =>
                {
                    ...
                    var sc1 = (newCounters1[s] != null ? newCounters1[s] : null);
                    var tc1 = (newCounters1[t] != null ? newCounters1[t] : null);
                    ...
                    if (s == t)
                    {
                        ...
                        if (sc1 != null)
                        {   // EXIT on loop means final state
                            if (ops.Length == 1 && (ops[0].OperationKind == CounterOp.EXIT))
                    ...
                    else if (sc1 != null && tc1 == null)
                    {
                        //this is a nonmonadic counting loop - really?
                        // ordinary exit from counting state
                        if (ops.Length == 1 && (ops[0].OperationKind == CounterOp.EXIT))
                            ops1.Add(new CounterOperation(sc1, CounterOp.EXIT));
                        else if (ops.Length == 1 && ops[0].OperationKind == CounterOp.INCR)
                            ops1.Add(new CounterOperation(sc1, CounterOp.INCR));
                    ...
                    else
                    {
                        foreach (var op in ops)
                        {
                            ...
                            {
                                if (!op.Counter.Equals(tc1))
                                    throw new AutomataException(AutomataExceptionKind.InternalError);
                                // entrence to the loop
                                if (sc1 == null && tc1 != null)
                                    ops1.Add(new CounterOperation(tc1, CounterOp.SET1));
                                else
                                    ops1.Add(new CounterOperation(tc1, CounterOp.INCR));
                            }
                            else if (op.OperationKind == CounterOp.EXIT)
                            {
                                if (ops.Length > 1)
                                    throw new AutomataException(AutomataExceptionKind.InternalError);
            ...
            {
                countingStates = null;
                #region computation of counting states
                //remap counters to fresh ones
                ICounter[] newCounters1 = new ICounter[regexLookup.Count];
                ...
                else
                {
                    Func<int, int, Sequence<CounterOperation>, Sequence<CounterOperation>> F = (s, t, ops) =>
                    {
                        #region function set operations for counters
                        var sc1 = (newCounters1[s] != null ? newCounters1[s] : null);
                        var tc1 = (newCounters1[t] != null ? newCounters1[t] : null);
                        var ops1 = new List<CounterOperation>();
                        ...
                        if (s == t)
                        {
                            ...
                            {   // EXIT on loop means final state
                                ...
                                {
                                    ops1.Add(new CounterOperation(sc1, CounterOp.EXIT));
                                    ops1.Add(new CounterOperation(sc1, CounterOp.SET0));
                                }
                                else if (ops.Length == 1 && ops[0].OperationKind == CounterOp.EXIT_SET1)
                                    ops1.Add(new CounterOperation(sc1, CounterOp.EXIT_SET1));
                                else if (ops.Length == 1 && ops[0].OperationKind == CounterOp.INCR)
                                    ...
                                    throw new AutomataException("Error: Creating loop in NCA.");
                            }
                        }
                        else if (sc1 != null && tc1 == null)
                        {
                            ...
                            {
                                // TODO: can it happend if the expression is monadic?
                                if (level == pushLevel)
                                {
                                    ops1.Add(new CounterOperation(sc1, CounterOp.INCR));
                                }
                                // add target state to counting states
                                var bc = new BoundedCounter(sc1.CounterId, sc1.LowerBound, sc1.UpperBound);
                                newCounters1[t] = bc;
                            }
                            ...
                            {
                                throw new AutomataException("Error: EXIT_SET1 leading to noncounting state.");
                            }
                            else
                            {
                                // TODO: NOOP in the scope or outside the scope - but then the source would not be counting state
                                // noop inside a bigger loop
                                if (level == pushLevel)
                                {
                                    ops1.Add(new CounterOperation(sc1, CounterOp.INCR));
                                }
                        ...
                        else
                        {
                            foreach (var op in ops)
                            {
                                ...
                                {
                                    // TODO: one cannot test it because there is no sc for some cases
                                    // if (!op.Counter.Equals(sc))
                                    //   throw new AutomataException(AutomataExceptionKind.InternalError);

                                    if ((sc1 != null && tc1 != null) && sc1 != tc1) // exit to different counting state
                                    {
                                        ops1.Add(new CounterOperation(sc1, CounterOp.EXIT));
                                    }
                                    else if ((sc1 != null && tc1 != null) && sc1 == tc1) // RESET in the bigger loop
                                    {
                                        ops1.Add(new CounterOperation(sc1, CounterOp.EXIT));
                                        ops1.Add(new CounterOperation(sc1, CounterOp.SET0));
                                    }
                                ...
                                {   // if non-monadic then it is a part of bigger loop
                                    // if (!op.Counter.Equals(tc))
                                    //   throw new AutomataException(AutomataExceptionKind.InternalError);
                                    // entrence to the loop
                                    if ((sc1 == null && tc1 != null))
                                        ops1.Add(new CounterOperation(tc1, CounterOp.SET1));
                                    else if ((sc1 != null && tc1 != null) && sc1 == tc1)
                                    {
                                        // bigger loop NOOP or INCR 
                                        if (level == pushLevel)
                                        {
                                            ops1.Add(new CounterOperation(sc1, CounterOp.INCR));
                                        }
                                    ...
                                    {
                                        // entrence to the loop from different counting state
                                        ops1.Add(new CounterOperation(tc1, CounterOp.SET1));
                                    }
                                    else
                                    {
                                ...
                                else if (op.OperationKind == CounterOp.EXIT_SET1)
                                {
                                    ...
                                    if ((sc1 != null && tc1 != null)) // exit to different counting state or RESET1 in the bigger loop
                                    {
                                        ops1.Add(new CounterOperation(sc1, CounterOp.EXIT));
                                        ops1.Add(new CounterOperation(tc1, CounterOp.SET1));
                                    }
                                    else
                                        throw new AutomataException("Error: EXIT_SET1 to noncounting state.");
                                }
                                else
                                {
                                    throw new AutomataException("Error: Unkown operation in creating NCA.");
                                }
                            }
                        }
                        var seq = new Sequence<CounterOperation>(ops1.ToArray());
                        if (s != t && tc1 != null && sc1 == null)
                        {
                            if (!seq.Exists(x => x.Counter.CounterId == tc1.CounterId))
                            {
                                //t is a counting state but its counter is not being initialized
                    ...
                    var allStates = new List<int>(cs);
                    List<int> next = new List<int>(cs);
                    List<int> current = null;
                    // iterate until new counters are added
                    // while (addList.Count != 0 || current == null)
                    while (next.Count != 0)
                    {
                        current = next;
                        next = new List<int>();
                        // iterate over a set of states 
                        foreach (var s in current)
                        {
                            foreach (var move in moves)
                            {
                                // select outgoing edges from state
                                if (move.SourceState == s)
                                {
                                    // if target state now proceeded
                                    if (!allStates.Contains(move.TargetState))
                                        next.Add(move.TargetState);
                                    // compute operations for the edge
                                    var ops = F(move.SourceState, move.TargetState, move.Label.Item2);
                                    // create a label of the edge
                                    var lab = new Tuple<Maybe<S>, Sequence<CounterOperation>>(move.Label.Item1, ops);
                                    // add move to a set of moves
                                    var move1 = Move<Tuple<Maybe<S>, Sequence<CounterOperation>>>.Create(move.SourceState, move.TargetState, lab);
                                    moves1.Add(move1);
                                }
                            }
                        }
                        // remember all discovered states
                        allStates.AddRange(next);
                        level++;
                    }
                    ...
                    foreach (var move in moves)
                    {
                        ...
                        if (!allStates.Contains(move.SourceState))
                        {
                            var ops = F(move.SourceState, move.TargetState, move.Label.Item2);
                            var lab = new Tuple<Maybe<S>, Sequence<CounterOperation>>(move.Label.Item1, ops);
                            var move1 = Move<Tuple<Maybe<S>, Sequence<CounterOperation>>>.Create(move.SourceState, move.TargetState, lab);
        ...
        public CountingAutomaton<S> CreateCountingAutomaton(bool makeMonadic = true)
        {
            /* SymbolicRegexNode<S> node = (makeMonadic ? this.MkMonadic() : this);
             ...
             List<ICounter> counters = null;
             var aut = node.Explore(out countingStates, out stateMap, makeMonadic);
             var ca = new CountingAutomaton<S>(aut, stateMap, countingStates, counters);
             return ca;*/
            return null;
        }
        ...
        /// </summary>
        /// <param name="makeMonadic">if true then nonmonadic loops are removed</param>
        public CountingAutomaton<S> CreateCountingAutomatonPushIncr(bool makeMonadic = true, int pushLevel = 0)
        {
            SymbolicRegexNode<S> node = (makeMonadic ? this.MkMonadic() : this);
            ICounter[] countingStates;
            SymbolicRegexNode<S>[] stateMap;
            List<ICounter> cnts;
            var aut = node.Explore(out countingStates, out stateMap, out cnts, makeMonadic, pushLevel);
            var ca = new CountingAutomaton<S>(aut, stateMap, countingStates, cnts);
            return ca;
        }
        ...
        /// </summary>
        public HashSet<S> GetPredicates(string filter = "")
        {
            var predicates = new HashSet<S>();
            CollectPredicates_helper(predicates, filter);
            return predicates;
        }
        ...
        /// </summary>
        void CollectPredicates_helper(HashSet<S> predicates, string filter = "")
        {
            switch (kind)
            {
                case SymbolicRegexKind.StartAnchor:
                ...
                case SymbolicRegexKind.Singleton:
                    {
                        //Console.WriteLine(this.set);
                        if (filter != string.Empty)
                        {
                            var pred_bdd = this.builder.solver.CharSetProvider.MkCharSetFromRegexCharClass(filter);
                            var pred_S = this.builder.solver.ConvertFromCharSet(pred_bdd);                            
                            this.set = builder.solver.MkAnd(this.set, pred_S);
                        }
                ...
                case SymbolicRegexKind.Loop:
                    {
                        this.left.CollectPredicates_helper(predicates, filter);
                        return;
                    }
                case SymbolicRegexKind.Or:
                case SymbolicRegexKind.And:
                    {
                        foreach (SymbolicRegexNode<S> sr in this.alts)
                            sr.CollectPredicates_helper(predicates, filter);
                        return;
                    }
                case SymbolicRegexKind.Concat:
                    {
                        left.CollectPredicates_helper(predicates, filter);
                        right.CollectPredicates_helper(predicates, filter);
                        return;
                    }
                default: //ITE
                    {
                        this.iteCond.CollectPredicates_helper(predicates, filter);
                        this.left.CollectPredicates_helper(predicates, filter);
                        this.right.CollectPredicates_helper(predicates, filter);
                        return;
                    }
            }
        }
        ...
        /// </summary>
        public S[] ComputeMinterms(string filter = "")
        {
            // get all predicates of the regex
            var predicates = new List<S>(GetPredicates(filter));
            // create minterms
        ...
        internal SymbolicRegexSampler<S> GenerateRandomMemberPartially(int maxLoopUnrol = 10, int cornerCaseProb = 5, int maxSamplingIter = 3, int stopProb = 0)
        {
            var sampler = new SymbolicRegexSampler<S>(this.builder, this, maxLoopUnrol, cornerCaseProb, maxSamplingIter, stopProb);
            return sampler;
        }
        ...
        /// </summary>
        /*  internal SymbolicRegexNode<S> Simplify(bool inner = false)
          {
              switch (kind)
              {
                  case SymbolicRegexKind.EndAnchor:
                  ...
                  case SymbolicRegexKind.WatchDog:
                      return this;
                  case SymbolicRegexKind.Concat: 
                      return builder.MkConcat(left.Simplify(inner), right.Simplify(inner));
                  case SymbolicRegexKind.Or:
                      return builder.MkOr(alts.Simplify(inner));
                  case SymbolicRegexKind.And:
                      return builder.MkAnd(alts.Simplify(inner));
                  case SymbolicRegexKind.Loop:
                      {
                          var body = this.left.Simplify(true);
    ...
    public class SymbolicRegexSet<S> : IEnumerable<SymbolicRegexNode<S>>
    {
        ...
        internal SymbolicRegexSet<S> Simplify(bool inner = false)
        {
            if (kind == SymbolicRegexSetKind.Disjunction)
                return CreateDisjunction(builder, SimplifyElems(inner));
            else
                return CreateConjunction(builder, SimplifyElems(inner));
        }

        IEnumerable<SymbolicRegexNode<S>> SimplifyElems(bool inner = false)
        {
            foreach (var elem in this)
                yield return elem.Simplify(inner);
        }
