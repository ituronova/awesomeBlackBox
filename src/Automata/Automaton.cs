...
namespace Microsoft.Automata
{
    ...
    public class Automaton<T> : IAutomaton<T>
    {
      ...
      /*  public void SetWeight<T>(int s, int t, int w, CsAlgebra<T> alg)
        {
            delta[s].Add((Move<T>)(Move<CsLabel<T>>.Create(s, t, CsLabel<T>.MkWTrans(alg, w))));
        }*/
        ...
        public int CountTrans()
        {
            ...
            foreach (int state in States)
            {
                ...
                {
                    if (!listTargets.Contains(move.TargetState))
                        listTargets.Add(move.TargetState);
                    continue;
                }
                count += listTargets.Count;
            }
            return count;
        }
        ...
        public virtual int NrOfCounters
        {
            get { return this.cntCount; }
        }

        public List<Move<T>> GetAllMoves()
        {
            var listOfMoves = new List<Move<T>>();
            foreach (int state in States)
                foreach (Move<T> move in delta[state])
