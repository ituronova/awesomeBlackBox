...
namespace Microsoft.Automata
{
    ...
    public class SymbolicRegex<S> : RegexMatcher
    {
        ...
        [NonSerialized]
        bool A_endAnchor = false;


        public void GetAnchors(string rgx)
        {
            if (rgx[0] == '^')
                A_startAnchor = true;
            if (rgx[rgx.Length - 1] == '$')
        ...
        int Delta(int c, int q, out SymbolicRegexNode<S> regex, out bool miss)
        {
            int p;
            miss = false;
            #region copy&paste region of the definition of Delta being inlined
        ...
        override public Tuple<int, int>[] Matches(string input, int limit = 0, int startat = 0, int endat = -1)
        {
            ...
            while (true)
            {
                ...
                i = (A_has_nonempty_prefix ?
                        ...
                        FindFinalStatePosition(input, k, i, out i_q0_A1, out watchdog, out misses, out hits));
        ...
        /// </summary>
        public bool IsMatch(string input, out int misses, out int hits, int startat = 0, int endat = -1)
        {
            misses = 0;
            hits = 0;
            ...
            else
            {
                ...
                {   // main function
                    i = FindFinalStatePosition(input, k, startat, out i_q0, out watchdog, out misses, out hits);
                }
                if (i == k)
                {
        ...
        /// unspecified and taken to be input.Length-1</param>
        /// </summary>
        public bool IsMatchCA(string input, CsAutomatonOpt<S> aut, int startat = 0, int endat = -1)
        {
            //return false;
            if (input == null || startat >= input.Length || startat < 0)
            {
            ...
            else
            {
                ...
                if (this.A_prefix != string.Empty)
                {
                    i = FindFinalStatePositionCAOpt(input, aut, k, startat);
                }
                else
                {   // main function
                    i = FindFinalStatePositionCA(input, aut, k, startat);
                }
                if (i == k)
                {
                    //the search for final state exceeded the input, so final state was not found
        ...
        private int FindFinalStatePosition(string input, int k, int i, out int i_q0, out int watchdog, out int misses, out int hits)
        {
            int q = q0_A1;
            int i_q0_A1 = i;
            ...
            while (i < k)
            {
                ...
                p = Delta(c, q, out regex, out miss);
#endif
                if (miss)
                    misses++;
                else
        ...
        /// <param name="i_q0">last position the initial state of A1 was visited</param>
        /// <param name="k">input length or bounded input length</param>
        private int FindFinalStatePositionCA(string input, CsAutomatonOpt<S> aut, int k, int i)
        {
            // initialize init state and initial state of the counting sets

            // initial state
            if(aut.currentState != -1)
            {
                if (aut.cntCount != 0)
                {   // initialize counting sets
                    for (int j = 0; j < aut.cntCount; j++)
                    {
                        if (!aut.cs[j].IsEmpty())
                            aut.cs[j].Clear();    // initialization to 0   
                    }
                }
            }            
            aut.currentState = aut.InitialState;
            
            /*if(caching)
            {
                var flags = new int[aut.cntCount];
                for (int j = 0; j < aut.cntCount; j++)
                {
                    flags[j] = aut.cs[j].GetHashCode();
                }
                nt = new Tuple<int, int[]>(aut.currentState, flags);
                matcher.ht.Add(nt, null);
            }*/
            ...
            {
                var c = input[i];
                var symbol = (aut.precomputed.Length > c) ? (aut.precomputed[c]) : aut.inputAlgebra.MintermIndex(atoms[dt.bst.Find(c)]);
               ...
               /* if (caching)
                {
                    int key = aut.currentState << 4 * aut.cntCount;
                    for (int j = 0; j < aut.cntCount; j++)
                    {
                        key |= aut.cs[j].flag << 4 * j;
                    }
                   // nt = new Tuple<int, int[]>(aut.currentState, flags);
                   // if(!matcher.ht.ContainsKey(key))
                     //   matcher.ht.Add(key, nt);
                }*/
             #endif
        ...
        /// <param name="i_q0">last position the initial state of A1 was visited</param>
        /// <param name="k">input length or bounded input length</param>
        private int FindFinalStatePositionCAOpt(string input, CsAutomatonOpt<S> aut, int k, int i)
        {
            int q = q0_A1;
            int i_q0_A1 = i;
            ...
            while (i < k)
            {
                ...
                if (q == q0_A1)
                {
                    ...
                    else
                    {
                        ...
                        // ---
                        q = this.A1_skipState;
                        var regex = this.A1_skipStateRegex;
                ...
                var symbol = aut.inputAlgebra.MintermIndex(aut.inputAlgebra.MkCharConstraint(input[i]));
 #if INLINE
                #region copy&paste region of the definition of Delta being inlined
                int atom_id = (dt.precomputed.Length > c ? dt.precomputed[c] : dt.bst.Find(c));
                S atom = atoms[atom_id];
...
# endif
        ...
        public Tuple<int, int, string> GenerateRandomMatchPartially(int maxLoopUnrol = 10, int cornerCaseProb = 5, int maxSamplingIter = 3, int stopProb = 0)
        {
            var sampler = A.GenerateRandomMemberPartially(maxLoopUnrol, cornerCaseProb, maxSamplingIter, stopProb);
            string str = sampler.GenerateRandomMember();
            // TODO: get numbr of done and not-done
            return new Tuple<int, int, string>(sampler.done, sampler.notDone, str);
        }
        #endregion
    }
}
