...
namespace Automata.Tests
{
    ...
    public class CountingSetTests
    {
     /*   [TestMethod]
        ...
        { 
            var set = new BasicCountingSet[1];
            set.Set0();
            set.IncrPush0();
            Assert.IsTrue(!set.IsEmpty);
            Assert.IsTrue(!set.IsSingleton);
        ...
        [TestMethod]
        public void TestBasicCountingSet_InrcPush2()
        {
            ...
            Assert.IsTrue(set.Max == 1);
            set.Incr();
            Assert.IsTrue(set.ToString() == "[2,1]");
            Assert.IsTrue(set.Max == 2);
            set.Incr();
            Assert.IsTrue(set.ToString() == "[3,2]");
            Assert.IsTrue(set.Max == 3);
            set.Incr();
            Assert.IsTrue(set.ToString() == "[4,3]");
            Assert.IsTrue(set.Max == 4);
            Assert.IsTrue(set.Min == 3);
            set.IncrPush0();
            Assert.IsTrue(set.ToString() == "[5,4,0]");
            Assert.IsTrue(set.Max == 5);
            set.Incr();
            Assert.IsTrue(set.ToString() == "[6,5,1]");
            Assert.IsTrue(set.Max == 6);
            set.Incr();
            Assert.IsTrue(set.ToString() == "[7,6,2]");
            Assert.IsTrue(set.Max == 7);
            ...
            set.Incr();
            set.Incr();
            Assert.IsTrue(set.ToString() == "[6]");
            Assert.IsTrue(set.Max == 6);
            Assert.IsTrue(set.IsSingleton);
            set.IncrPush01();
            Assert.IsTrue(!set.IsEmpty);
            Assert.IsTrue(set.ToString() == "[7,1,0]");
            set.Incr();
            set.Incr();
            set.Incr();
            set.Incr();
